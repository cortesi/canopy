...
impl Lua {
    ...
    pub(crate) unsafe fn configure_luau(&self) -> Result<()> {
        ...
        // Installing mlua's default Luau `require` currently fails on some builds because
        // the upstream bootstrap misreads stack values during initialization. Expose a
        // stable placeholder instead; mlua's `create_require_function()` can be revisited
        // once the upstream bug is fixed.
        self.globals()
            .raw_set("require", self.create_c_function(lua_require_disabled)?)?;
...
unsafe extern "C-unwind" fn lua_require_disabled(state: *mut ffi::lua_State) -> c_int {
    ffi::luaL_error(
        state,
        cstr!("require is unavailable in this build"),
    )
}
