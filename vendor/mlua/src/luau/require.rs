...
pub(super) fn create_require_function<R: Require + MaybeSend + 'static>(
    ...
    let (get_cache_key, find_current_file, proxyrequire, registered_modules, loader_cache) =
        lua.exec_raw_lua(|lua| unsafe {
            let state = lua.state();
            ...
            Ok::<_, Error>((
                stack_function(lua, base + 1)?,
                stack_function(lua, base + 2)?,
                stack_function(lua, base + 3)?,
                stack_table(lua, base + 4)?,
                stack_table(lua, base + 5)?,
            ))
        })?;
    ...
    let (error, r#type, to_lowercase) = lua.exec_raw_lua(|lua| unsafe {
        let state = lua.state();
        let _sg = StackGuard::new(state);
        let base = ffi::lua_gettop(state);
...
unsafe fn stack_function(lua: &crate::state::RawLua, idx: c_int) -> Result<Function> {
    let value = lua.stack_value(idx, None);
    match value {
...
unsafe fn stack_table(lua: &crate::state::RawLua, idx: c_int) -> Result<Table> {
    let value = lua.stack_value(idx, None);
    match value {
