...
fn assert_buffer_matches(render: &PartRender, expected: &[&str]) {
    render.get_buffer().assert_matches(expected);
}
...
impl BufTest {
    ...
    fn run(self) {
        ...
        let mut render = PartRender::new(&stylemap, &mut style_manager, canvas_size, render_rect);
