...
pub mod canopy_mcp {
    ...
    where
        F: 'static + Fn() -> crate::Result<canopy::Canopy> + Send + Sync, {
    }
    ...
    impl Clone for AppEvaluator {
        fn clone(&self) -> AppEvaluator {}
    }
    ...
    impl Clone for AutomationPolicy {
        fn clone(&self) -> AutomationPolicy {}
    }

    impl Debug for AutomationPolicy {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {}
    }

    impl Default for AutomationPolicy {
        fn default() -> AutomationPolicy {}
    }

    impl Eq for AutomationPolicy {
        #[doc(hidden)]
        fn assert_fields_are_eq(&self) {}
    }

    impl PartialEq for AutomationPolicy {
        fn eq(&self, other: &AutomationPolicy) -> bool {}
    }
    ...
    impl PartialEq for LaunchOptions {
        fn eq(&self, other: &LaunchOptions) -> bool {}
    }

    impl Clone for LiveContext {
        fn clone(&self) -> LiveContext {}
    }

    impl Debug for LiveContext {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {}
    }
