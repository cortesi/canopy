...
fn start_app(name: &str) -> canopy::tutils::PtyApp {
    let db = temp_db(name);
    open_store(db.to_str().unwrap()).unwrap();
    let mut app = spawn_workspace_bin("todo", &[db.to_str().unwrap()]).unwrap();
...
fn delete_after_moving_focus() {
    let mut app = start_app("move_del");
...
fn delete_first_without_nav() {
    let mut app = start_app("del_first");
...
#[test]
fn delete_middle_item_shows_following() {
    let mut app = start_app("middle_del");
    ...
    // The third item should still be visible and highlighted
