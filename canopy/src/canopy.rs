    ...
    pub(crate) fn mouse(&mut self, root: &mut dyn Node, m: mouse::MouseEvent) -> Result<()> {
        ...
        if let Some(nid) = node_at(root, m.location) {
            walk_to_root(root, &nid, &mut |x| {
                ...
                match hdl {
                    er if er.outcome == EventOutcome::Handle => {
                        handled = true;
                        if !er.no_render {
                            self.taint(x);
                        }
...
mod tests {
    ...
    fn tkey_no_render() -> Result<()> {
        ...
        use crate::{EventResult, NodeState, Error};
        ...
        impl Node for N {
            ...
            fn handle_key(&mut self, _c: &mut dyn Context, _k: key::Key) -> Result<EventResult> {
                Ok(EventResult::handled().no_render())
            }
        }
