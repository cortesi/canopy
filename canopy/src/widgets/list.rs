...
mod tests {
    ...
    #[test]
    fn no_overdraw_after_scroll() -> Result<()> {
        const SAMPLE: &str = "aaa bbb ccc\nddd";
        ...
        impl Node for Root {
            ...
            fn layout(&mut self, l: &Layout, sz: Expanse) -> Result<()> {
                l.fill(self, sz)?;
                let vp = self.vp();
                let view = Rect::new(1, 1, sz.w - 2, sz.h - 2);
                l.place(&mut self.frame, vp, view)?;
                Ok(())
            }
        }

        let size = Expanse::new(12, 6);
        ...
        canopy.set_root_size(size, &mut root)?;
        canopy.render(&mut cr, &mut root)?;
        let frame_rect = root.frame.vp().screen_rect();
        ...
        canopy.taint_tree(&mut root);
        canopy.render(&mut cr, &mut root)?;
        let final_buf = buf.lock().unwrap().cells.clone();

        for y in 0..size.h {
            for x in 0..size.w {
                if !frame_rect.contains_point(Point { x, y }) {
                    assert_eq!(final_buf[y as usize][x as usize], ' ');
                }
            }
        }
