...
mod tests {
    ...
    fn rects<N: Node + ListItem>(list: &mut List<N>) -> Vec<Rect> {
        let mut r = vec![];
        list.children(&mut |n| {
            if !n.is_hidden() {
                r.push(n.vp().screen_rect());
            }
            Ok(())
        })
        .unwrap();
    ...
    fn assert_non_overlapping(rects: &[Rect], bounds: Rect) {
        let mut prev: Option<Rect> = None;
        for r in rects {
            assert!(bounds.contains_rect(r));
            if let Some(p) = prev {
                assert!(p.tl.y + p.h <= r.tl.y);
            }
            prev = Some(*r);
        }
    }
    ...
    #[test]
    fn list_items_do_not_overlap_initial() -> Result<()> {
        use crate::backend::test::CanvasRender;
        const SAMPLE: &str = "aaa bbb ccc\nddd";
        ...
        let rects = rects(&mut root.frame.child);
        assert_non_overlapping(&rects, root.frame.child.vp().screen_rect());

        Ok(())
    }

    #[test]
    fn list_items_do_not_overlap_after_scroll() -> Result<()> {
        use crate::backend::test::CanvasRender;
        const SAMPLE: &str = "aaa bbb ccc\nddd";
